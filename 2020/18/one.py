...
def evaluate_expression(string):
    ...
    if f.isnumeric():
        ...
        f = evaluate_expression(extract_paranthesis(string))
        string = string.replace("(" + extract_paranthesis(string) + ")", "", 1).strip()
    else:
    ...
    if s.isnumeric():
        ...
        s = evaluate_expression(extract_paranthesis(string))
        ev = eval(" ".join([f, o, s]))
        return evaluate_expression(
            str(ev)
            + " "
            + string.replace("(" + extract_paranthesis(string) + ")", "", 1)
        )
    else:
        raise Exception(string)
